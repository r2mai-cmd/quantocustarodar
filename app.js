...
function slotCard(i){const id=slots[i],v=id?VEHICLES.find(x=>x.id===id):null;return `<article class="car-card ${v?'filled':'empty'}" data-slot="${i}">${v?`<button type="button" class="remove" data-remove="${i}" aria-label="Remover carro" title="Remover este carro">×</button>${photoMarkup(v)}<h3>${esc(carFullName(v))}</h3><div class="version">${esc(v.type||'PBEV 2026')} · PBEV 2026</div><div class="car-price" data-fipe-price="${esc(v.id)}">Preço de referência (FIPE) · carregando…</div><button class="spec-link" data-spec="${v.id}">Ficha técnica →</button>`:`<div class="car-photo"><button class="select-plus" data-select="${i}" aria-label="Selecionar carro">+</button></div><h3>Selecionar carro</h3><p class="empty-copy">Escolha marca, modelo e versão.</p><button class="select-button" data-select="${i}">Selecionar</button>`}</article>`;}
