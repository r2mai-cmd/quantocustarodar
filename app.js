...
function photoMarkup(v, cls='car-photo'){
  ...
  return `<div class="${cls}"><img src="${esc(v.imageUrl)}" alt="${esc(carName(v))}" loading="lazy" referrerpolicy="no-referrer" onerror="this.onerror=null;this.replaceWith(Object.assign(document.createElement('div'),{className:'car-placeholder',textContent:'🚗'}));"></div>`;
}
...
function cacheKey(v){return `qcr-photo-v4:${carName(v).toLowerCase()}`;}
